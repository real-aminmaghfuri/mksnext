  ...
  return (
    <aside className={`hidden lg:flex h-screen flex-col bg-white dark:bg-zinc-950 border-r border-zinc-200 dark:border-zinc-800 transition-all duration-500 ${collapsed ? 'w-16' : 'w-64'}`}>
        ...
        <div className="flex items-center gap-3">
          ...
          {!collapsed && (
            <span className="font-black text-lg tracking-tighter">
              MKS<span className="text-brand-600">.MEDIA</span>
            </span>
          )}
        </div>
      </div>
      ...
      <nav className="flex-1 p-4 space-y-1">
        {!collapsed && <p className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em] mb-3 ml-2 mt-2">Content Ops</p>}
        ...
        <div className="mt-8 border-t border-zinc-100 dark:border-zinc-800 pt-4">
            ...
            >
                <Globe size={18} />
                {!collapsed && <span className="ml-3 font-bold text-sm">View Website</span>}
            </Link>
        </div>
      </nav>

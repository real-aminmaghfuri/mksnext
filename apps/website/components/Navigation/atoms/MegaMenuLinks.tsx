...
export const MegaMenuLinks: React.FC<MegaMenuLinksProps> = ({ parentLabel, items, onLinkClick }) => {
  return (
    <div className="w-full">
      ...
      {/* 
         Horizontal Flow Priority:
         - grid-cols-1 md:grid-cols-3 : Forces 3 columns layout
         - gap-x-8 : Space between columns
         - gap-y-6 : Space between rows (compact)
      */}
        ...
        {items.map((item, itemIdx) => {
          ...
          return (
            <Link 
              ...
              href={item.path}
              onClick={onLinkClick}
              className="group flex items-start gap-3 transition-transform active:translate-x-1"
            >

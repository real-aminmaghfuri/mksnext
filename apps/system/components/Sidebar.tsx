  ...
  return (
    <aside className={`hidden md:flex h-screen sticky top-0 bg-white dark:bg-zinc-950 border-r border-zinc-300 dark:border-zinc-800 z-40 transition-all duration-300 flex-col justify-between ${collapsed ? 'w-20' : 'w-72'}`}>
        ...
        <nav className="p-4 space-y-2">
          {menuItems.map((item, idx) => {
            ...
            return (
              ...
              >
                <Icon className={`w-5 h-5 ${item.active ? 'text-brand-600 dark:text-brand-400' : ''}`} />
                {!collapsed && (
      ...
      <div className="p-4 border-t border-zinc-300 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50">
        <a href="/" className="w-full flex items-center p-3 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 border border-transparent hover:border-red-200 dark:hover:border-red-800 rounded-xl transition-all">
